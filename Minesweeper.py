...
class MineSweeper:
    ...
    def __init__(self,size_y,size_x,mine_ratio):
        ...
        self.NUM_MINES = int(self.y*self.x*mine_ratio)
...
while not done:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            screen.blit(myfont.render('20 x 20 (80 mines)',True,black),(100,300))
            screen.blit(myfont.render('25 x 20 (100 mines)',True,black),(100,350))
            screen.blit(myfont.render('30 x 25 (150 mines)',True,black),(100,400))
            if event.type == pg.KEYDOWN and event.key == pg.K_DOWN:
                choice = (choice + 1) % 4
            elif event.type == pg.KEYDOWN and event.key == pg.K_UP:
                choice = (choice - 1) % 4
            screen.blit(myfont.render('★',True,black),(60,250+50*choice))
            if event.type == pg.KEYDOWN and event.key == pg.K_RETURN:
                if choice == 0:
                    game_y,game_x,mine_ratio = 20,15,0.15
                elif choice == 1:
                    game_y,game_x,mine_ratio = 20,20,0.2
                elif choice == 2:
                    game_y,game_x,mine_ratio = 25,20,0.2
                else:
                    game_y,game_x,mine_ratio = 30,25,0.2
            ...
            starting_screen = True
            screen = pg.display.set_mode([500,500])
        elif not starting_screen and event.type == pg.KEYDOWN and event.key == pg.K_SPACE:
            game = MineSweeper(game_y,game_x,mine_ratio)
            
        
    if not starting_screen: game.display_board()
    pg.display.flip()
